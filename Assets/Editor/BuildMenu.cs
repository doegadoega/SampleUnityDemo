...
namespace SampleUnityDemo.Editor
{
    static class BuildMenu
    {
        ...
        /// iOSビルドの実行(Deviceのみ)
        /// </summary>
        [MenuItem("Build/Build iOS (Device Only)")]
        static void BuildIOSForDeviceOnly()
        {
            // デバイス向けビルドを実行
            bool ret = BuildIOSInternal(iOSSdkVersion.DeviceSDK);
            if (!ret)
            {
                return;
            }

            Debug.Log("<color=green>Build iOS (Device Only) Completed Successfully</color>");
        }
